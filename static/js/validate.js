...
$.extend($.validator, {
	...
	messages: {
		...
		tel: "Please fix this field.",
		idcard: "Please fix this field.",
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					/*删除 有BUG*/
					//$('#'+element.name).parent().next().find('aside').remove();
					if ( valid ) {
						...
						//$('#'+element.name).parent().next().find('aside').removeClass("reg_false").addClass("reg_true").text(message);
		...
		idcard: function(value, element) {
		   if(this.optional(element))
            ...
            //if(!value || !/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(value)){
               ...
               // pass = false;
            //}
            if(!value || /^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$/.test(value)){
               tip = "身份证号格式错误";
               pass = false;
            }
		...
		tel: function(value, element) {
			return this.optional(element) || value.match(/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/);
		},
